...
This repository is configured to deploy on Render as a single Docker-based web service. The Docker image builds the React frontend, copies the generated `frontend/dist` bundle into the image, and serves it from FastAPI so the UI and API share the same Render URL.

### Recommended one-service Render setup

Use the root-level [render.yaml](/C:/Users/dp686/Desktop/ai-accounting-analytics-platform/render.yaml) Blueprint, which creates a single Docker web service named `ai-accounting-analytics-platform`.

If you prefer the Render dashboard manually:
...
- copies the frontend build output into the final image
- starts FastAPI with Uvicorn on Render's assigned port
...
With the single-service setup, the frontend and backend are both available from the same origin, so no separate frontend Render site or CORS origin configuration is required for normal browser use.
